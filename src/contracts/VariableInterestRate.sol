...
contract VariableInterestRate is IRateCalculator {
    ...
    uint64 private constant MIN_INT = 79_123_523; // 0.25% annual rate
    uint64 private constant MAX_INT = 146_248_476_607; // 10,000% annual rate
    uint256 private constant INT_HALF_LIFE = 43_200e36; // given in seconds, equal to 12 hours,
        // additional 1e36 to make math simpler
    ...
    /// @return _calldata abi.encode(uint32 MIN_UTIL, uint32 MAX_UTIL, uint32 UTIL_PREC, uint64
    /// MIN_INT, uint64 MAX_INT, uint256 INT_HALF_LIFE)
    ...
    /// @notice The ```getNewRate``` function calculates the new interest rate as a function of time
    /// and utilization
    ...
    {
        (uint64 _currentRatePerSec, uint256 _deltaTime, uint256 _utilization,) =
        ...
        if (_utilization < MIN_UTIL) {
            ...
            uint256 _decayGrowth =
                INT_HALF_LIFE + (_deltaUtilization * _deltaUtilization * _deltaTime);
            ...
            if (_newRatePerSec > MAX_INT) _newRatePerSec = MAX_INT;
        } else {
