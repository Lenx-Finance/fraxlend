...
contract LinearInterestRate is IRateCalculator {
    ...
    /// @return _calldata abi.encode(uint256 MIN_INT, uint256 MAX_INT, uint256 MAX_VERTEX_UTIL,
    /// uint256 UTIL_PREC)
    ...
    /// @notice The ```requireValidInitData``` function reverts if initialization data fails to be
    /// validated
    /// @param _initData abi.encode(uint256 _minInterest, uint256 _vertexInterest, uint256
    /// _maxInterest, uint256 _vertexUtilization)
        ...
        returns (uint64 _newRatePerSec)
    {
        ...
        if (_utilization < _vertexUtilization) {
            ...
            uint256 _slope =
                (((_maxInterest - _vertexInterest) * UTIL_PREC) / (UTIL_PREC - _vertexUtilization));
            _newRatePerSec = uint64(
                _vertexInterest + (((_utilization - _vertexUtilization) * _slope) / UTIL_PREC)
            );
        } else {
