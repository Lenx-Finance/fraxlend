...
/// @author Drake Evans (Frax Finance) github.com/drakeevans, modified from work by @Boring_Crypto
/// github.com/boring_crypto
/// @notice Provides a library for use with the VaultAccount struct, provides convenient math
/// implementations
...
library VaultAccountingLibrary {
        ...
        returns (uint256 shares)
    {
        if (total.amount == 0) {
            ...
            if (roundUp && (shares * total.amount) / total.shares < amount) shares = shares + 1;
        }
    }
    ...
    function toAmount(VaultAccount memory total, uint256 shares, bool roundUp)
        internal
        pure
        returns (uint256 amount)
    {
        if (total.shares == 0) {
            ...
            if (roundUp && (amount * total.shares) / total.amount < shares) amount = amount + 1;
        }
    }
}
